...
const options = ['Live 1:1', 'Group sessions', 'Pre-recorded videos', 'Self-Mode'] as const
type SessionType = typeof options[number]

export default function SessionTypeSelectionPage({ step = 23 }: { step?: number }) {
    const router = useRouter();
    const [selected, setSelected] = useState<SessionType>('Group sessions')
    const currentImage = getOnboardingImage(step);

    const onPrevious = () => router.push('/onboarding/wellness-modes')
    const onNext = () => router.push('/onboarding/time-session-length')
            ...
            <div className="container-responsive min-h-screen flex flex-col justify-center py-8">
                <div className="flex-1 flex flex-col items-center justify-center space-y-8">
                    ...
                    <h1 className="text-2xl font-semibold text-center text-gray-800 mb-2 px-4" style={{ fontSize: '28px', lineHeight: '1.2' }}>
                        Please select your preferred session type
                    </h1>

                    <div className="w-full max-w-md space-y-3">
                        <label className="block text-sm font-medium text-gray-700">Select session types</label>
