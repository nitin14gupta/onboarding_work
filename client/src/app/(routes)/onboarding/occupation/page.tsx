...
const options = ['Desk job', 'Field work', 'House work', 'Retired'] as const
type Occupation = typeof options[number]

export default function OccupationPage({ step = 12 }: { step?: number }) {
    const router = useRouter();
    const [selected, setSelected] = useState<Occupation>('Desk job')
    const currentImage = getOnboardingImage(step);

    const onPrevious = () => router.push('/onboarding/activity-level')
    const onNext = () => router.push('/onboarding/diet')
