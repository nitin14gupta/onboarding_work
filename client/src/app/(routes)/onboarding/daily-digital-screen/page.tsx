...
const options = ['Less than 1 hour', '1–3 hours', '3–5 hours', '5–8 hours', 'More than 8 hours'] as const
type ScreenUse = typeof options[number]

export default function DailyDigitalScreenPage({ step = 18 }: { step?: number }) {
    const router = useRouter();
    const [selected, setSelected] = useState<ScreenUse>('3–5 hours')
    const currentImage = getOnboardingImage(step);

    const onPrevious = () => router.push('/onboarding/sleep-experience')
    const onNext = () => router.push('/onboarding/health-warning')
            ...
            <div className="container-responsive min-h-screen flex flex-col justify-center py-8">
                <div className="flex-1 flex flex-col items-center justify-center space-y-8">
                    ...
                    <h1 className="text-2xl font-semibold text-center text-gray-800 mb-2 px-4" style={{ fontSize: '28px', lineHeight: '1.2' }}>
                        Daily Digital Screen Use
                    </h1>

                    <div className="w-full max-w-md space-y-3">
                        <label className="block text-sm font-medium text-gray-700">Screen Time / Phone Usage Daily</label>
