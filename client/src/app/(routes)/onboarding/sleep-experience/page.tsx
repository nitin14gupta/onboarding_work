...
const options = ['Very Poor', 'Poor', 'Average', 'Good', 'Excellent'] as const
type Quality = typeof options[number]

export default function SleepExperiencePage({ step = 17 }: { step?: number }) {
    const router = useRouter();
    const [selected, setSelected] = useState<Quality>('Average')
    const currentImage = getOnboardingImage(step);

    const onPrevious = () => router.push('/onboarding/resting-hours')
    const onNext = () => router.push('/onboarding/daily-digital-screen')
